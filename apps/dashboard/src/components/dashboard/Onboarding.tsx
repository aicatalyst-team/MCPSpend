...
export function Onboarding() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <Step number={3} state={step3} title="Install the proxy in front of your MCP server">
        {!hasKey ? (
          <p className="text-sm text-gray-500">Finish step 2 first.</p>
        ) : (
          <div>
            <p className="text-sm text-gray-400 mb-3">Pick where you run MCP. Copy the snippet — replace <code className="text-brand-300 text-xs">/path/to/folder</code> with a real directory.</p>
            ...
            <InstallSnippet tab={tab} apiKey={revealKey || `${newestKey.prefix}<paste-your-full-key>`} />
            <p className="mt-4 text-xs text-gray-500">
              After saving the config, restart the IDE / Claude Desktop. Make any tool call — within seconds it appears here in your dashboard.
            </p>
          </div>
        )}
      </Step>
